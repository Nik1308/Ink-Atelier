import { TATTOO_CONSENT_FORM_API_URL, UPLOAD_IMAGE_API_URL, CUSTOMER_API_URL, PIERCING_CONSENT_FORM_API_URL, GENERATE_CONSENT_PDF_URL } from "../api/endpoints";
 ...
 */
export const handleCustomerLookupForForm = async (customerData, setError) => {
  try {
...
export async function submitTattooConsentForm(form, setLoading, setError) {
  ...
  try {
    ...
    const customerId = await handleCustomerLookupForForm(form, setError);
