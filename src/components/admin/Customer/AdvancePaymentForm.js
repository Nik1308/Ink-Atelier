...
const AdvancePaymentForm = () => {
  ...
  const validateAdvance = () => {
    if (!advanceForm.name) return 'Customer name is required.';
    if (!advanceForm.phone.match(/^\d{10}$/)) return 'Enter a valid 10-digit phone number.';
    if (!advanceForm.appointmentDate) return 'Appointment date is required.';
    if (!advanceForm.advanceAmount || isNaN(advanceForm.advanceAmount) || Number(advanceForm.advanceAmount) < 1) return 'Enter a valid advance amount.';
    if (!advanceForm.totalQuotePrice || isNaN(advanceForm.totalQuotePrice) || Number(advanceForm.totalQuotePrice) < 1) return 'Enter a valid total quote price.';
    if (!advanceForm.service) return 'Select a service.';
    return null;
  };
